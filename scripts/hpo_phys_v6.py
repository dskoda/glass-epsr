#!/usr/bin/env python3
"""Physics-weighted HPO v6 — restart-capable denoising.

Forked from ``hpo_phys_v5.py``. v5 (1200 trials) found the best objective
at 37.91 but left ~8-10% of Si atoms undercoordinated. v6 introduces
``n_restart``: after each full tmax→tmin denoising pass the output structure
is used as the starting positions for the next pass (same cell, species, and
guidance). Restarts inherit the locally-bonded geometry and let the score
network re-resolve long-range topology.

Search space is intentionally tight (5 parameters). All non-restart
parameters are fixed at v5 best values. The v5 best is enqueued as trial 0
so TPE starts from a known-good point.

Fixed parameters (from v5 best trial #626, 1200-trial study):
    tmin=9.267e-3, tstep=256, t_rho=1.01
    n_corr=2, corr_step_size=0.44, corr_t_gate=0.464
    rho=416.0, N_anneal=0

Searched parameters (5):
    tmax ∈ [0.4, 1.0]
    tersoff_lambda ∈ [0.0, 0.4]
    tersoff_schedule ∈ {constant, linear, sigmoid}
    tersoff_t_gate ∈ [0.1, 0.8]
    n_restart ∈ {1, 2, 3, 4}

Objective is identical to v5 (8-term, log-scale undercoord).
...
Quick smoke (single GPU, 4 trials)::
    ...
    python scripts/hpo_phys_v6.py research/density_extrapolation/experiment/ \\
        ...
        --study-name glass_phys_v6_smoke \\
        --storage /tmp/glass_phys_v6_smoke.db

Full run (4 GPUs, 100 trials, ~2-4 h)::
    ...
    python scripts/hpo_phys_v6.py research/density_extrapolation/experiment/ \\
        ...
        --study-name glass_phys_v6_restart \\
        --storage research/hpo/glass_phys_v6_restart.db
"""
...
# --------------------------------------------------------------------------
# Objective weights — physical-correctness emphasis with log-scale
# under-coordination penalties. Goal is frac_le3 → 0 and frac_le2 → 0.
# --------------------------------------------------------------------------
...
# Fixed parameters from v5 best trial #626 (1200-trial glass_phys_v5_15ood study).
# These are not searched in v6; only tmax + tersoff + n_restart are free.
_FIXED_PARAMS: Dict = {
    "tmin": 9.267e-3,
    "tstep": 256,
    "t_rho": 1.01,
...
def _phys_obj(errors: Dict[str, float]) -> float:
    ...
    `errors` is the dict returned by ``compute_all_errors`` plus inline-
    added ``undercoord_frac_le3`` and ``undercoord_frac_le2`` keys.
    """
    return (
...
def _sample_params(trial) -> Dict:
    searched = {
        # --- Time schedule (only tmax is free; others fixed at v5 best) ---
        "tmax": trial.suggest_float("tmax", 0.4, 1.0),
        # --- Tersoff guidance ---
        "tersoff_lambda": trial.suggest_float("tersoff_lambda", 0.0, 0.4),
...
def _evaluate_one(
    ...
    # Undercoord fractions are absolute properties of the generated
    # structure (no reference). Compute inline; do not pollute
    # compute_all_errors.
    ...
    # Backwards-compat alias used by some downstream tooling.
...
def _build_objective(
    ...
    # Component error keys logged in user_attrs alongside the per-trial
    # mean. Order matches the objective above.
    _ERR_KEYS = (
        "pdf_rmse",
        "coordination_emd",
        "undercoord_frac_le3",
...
def main():
    ...
    parser.add_argument("--study-name", type=str, default="glass_phys_v6_restart")
    parser.add_argument("--storage", type=str,
                        default="research/hpo/glass_phys_v6_restart.db")
    ...
    # Warm-start: enqueue v5 best (trial #626, 1200-trial study) as the
    # first trial so TPE has a known-good anchor before random exploration.
    # Only enqueued when the study has zero trials (i.e., fresh start).
    if len(study.trials) == 0:
        study.enqueue_trial({
            "tmax": 0.595,
            "tersoff_lambda": 0.281,
            "tersoff_schedule": "sigmoid",
            "tersoff_t_gate": 0.276,
            "n_restart": 1,
        })
    ...
    for t in all_trials[:5]:
        ...
        print(
            ...
            f"ts={t.params.get('tersoff_schedule')} "
            f"tg={t.params.get('tersoff_t_gate'):.3f}"
        )
